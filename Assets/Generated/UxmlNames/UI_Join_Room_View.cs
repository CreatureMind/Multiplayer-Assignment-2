// AUTO-GENERATED — DO NOT EDIT
// Source : Assets/UI Toolkit/UXMLs/Views/Join_Room_View.uxml
// Updated: 2026-08-06 19:28 UTC

using System.Collections.Generic;

public enum UI_Join_Room_View
{
    tint,
    panel,
    title,
    room_code_field,
    error_label,
    back_button,
    join_button,
}

internal static class UI_Join_Room_View_Map
{
    internal static readonly Dictionary<UI_Join_Room_View, string> Names =
        new Dictionary<UI_Join_Room_View, string>
        {
            { UI_Join_Room_View.tint, "tint" },
            { UI_Join_Room_View.panel, "panel" },
            { UI_Join_Room_View.title, "title" },
            { UI_Join_Room_View.room_code_field, "room-code-field" },
            { UI_Join_Room_View.error_label, "error-label" },
            { UI_Join_Room_View.back_button, "back-button" },
            { UI_Join_Room_View.join_button, "join-button" },
        };
}
